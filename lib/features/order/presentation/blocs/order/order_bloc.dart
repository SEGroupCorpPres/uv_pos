...
//   Future<void> _fetchOrdersByDates(
//       FetchOrdersByDatesEvent event, Emitter<OrderState> emit) async {
//     try {
//       print('event.storeID is  ------>  ${event.storeID}');
//       emit(OrderLoading());
...
//       FetchOrderByIdEvent event, Emitter<OrderState> emit) async {
//     try {
//       emit(OrderLoading());
...
//         emit(OrderCreated(
//             order: createdOrder, storeID: event.storeID, uid: event.uid));
//         emit(OrdersFromDateByStoreIDLoaded(orders: orders));
...
//         final existingProductIndex = currentState.products!
//             .indexWhere((product) => product.id == event.product.id);
//         if (existingProductIndex != -1) {
//           final updatedProducts =
//               List<OrderProductModel>.from(currentState.products!);
...
//       RemoveProduct event, Emitter<OrderState> emit) async {
//     if (state is UpdatedOrderProducts) {
//       final state = this.state as UpdatedOrderProducts;
//       List<OrderProductModel> products = state.products!
//           .where((product) => product.id != event.productId)
//           .toList();
//       log('$products');
//       emit(UpdatedOrderProducts(
//           products: List<OrderProductModel>.from(products)));
//     }
//   }
...
//         log('$updatedProducts');
//         emit(UpdatedOrderProducts(
//             products: List<OrderProductModel>.from(updatedProducts)));
...
//           (await _orderRepository.getOrdersForDateByStoreId(
//         event.storeID,
//         event.order.orderDate,
//       ))
//               .cast<OrderModel>();
...
//       ClearProductList event, Emitter<OrderState> emit) async {
//     emit(const UpdatedOrderProducts(products: []));
//   }
// }
